...
type ShotSplit = { attempts: number; goals: number };
type Stats = { matches: number; shots: number; onTarget: number; goals: number; passTry: number; passSuccess: number; possession: number; routes: number[]; goalBuckets: number[]; shotMap: Array<{ x: number; y: number; goal: boolean }>; goalBreakdown?: { types: Record<string, ShotSplit>; locations: { inside: ShotSplit; outside: ShotSplit } } };
...
export default function AnalysisPage() {
  ...
  useEffect(() => { const player = new URLSearchParams(window.location.search).get("player"); if (player && data?.analytics?.[player]) setSelected(player); }, [data]);
  ...
  const efficiencyTable = useTriSort(ranking);
  const matchupTable = useTriSort(headToHead.combinations);
  ...
  const goalTypes = Object.entries(stats.goalBreakdown?.types || {}).sort(([, a], [, b]) => b.goals - a.goals || b.attempts - a.attempts);
  const locations = stats.goalBreakdown?.locations;
  return <main className="subpage"><FloatingNav />
    ...
    <section className="page-shell analysis-layout">
      ...
      <article className="clean-card span-three goal-profile"><div className="card-heading"><div><p>GOAL PROFILE</p><h2>득점 유형 분석</h2></div><span>득점 비율 · 시도 대비 성공률</span></div>
        <div className="goal-profile-grid"><div className="goal-type-list">{goalTypes.length ? goalTypes.map(([name, value]) => { const share = Math.round(value.goals / Math.max(1, stats.goals) * 100); const conversion = Math.round(value.goals / Math.max(1, value.attempts) * 100); return <div key={name}><strong>{name}</strong><span>{value.goals}골 <small>전체의 {share}%</small></span><i><em style={{width:`${share}%`}}/></i><b>{value.attempts}회 시도 · 성공률 {conversion}%</b></div>; }) : <p>집계할 슈팅 데이터가 없습니다.</p>}</div>
          <div className="goal-location"><div><span>박스 안</span><strong>{locations?.inside.goals || 0}<small>골</small></strong><b>{locations?.inside.attempts || 0}회 시도 · {Math.round((locations?.inside.goals || 0) / Math.max(1, locations?.inside.attempts || 0) * 100)}%</b></div><div><span>중거리</span><strong>{locations?.outside.goals || 0}<small>골</small></strong><b>{locations?.outside.attempts || 0}회 시도 · {Math.round((locations?.outside.goals || 0) / Math.max(1, locations?.outside.attempts || 0) * 100)}%</b></div></div></div>
        <p className="class-disclaimer">※ 중거리는 API의 페널티박스 밖 슈팅으로 계산합니다. 감아차기 중거리골처럼 기술 유형과 위치 유형은 서로 중복될 수 있습니다.</p>
      </article>
      <article className="clean-card span-three h2h-card"><div className="card-heading"><div><p>HEAD TO HEAD</p><h2>유저별 맞대결 분석</h2></div><div className="h2h-selectors"><select value={selected} onChange={(e) => setSelected(e.target.value)}>{Object.keys(data.analytics).filter((name) => name !== opponent).map((name) => <option key={name}>{name}</option>)}</select><span>VS</span><select value={opponent} onChange={(e) => setOpponent(e.target.value)}>{Object.keys(data.analytics).filter((name) => name !== selected).map((name) => <option key={name}>{name}</option>)}</select></div></div>
        ...
        <div className="class-matchups"><div className="class-head">{([['mine',`${selected} 주력 클래스`],['theirs',`${opponent} 주력 클래스`],['p','경기'],['w','승'],['gf','득점']] as Array<[keyof (typeof headToHead.combinations)[number],string]>).map(([key,label]) => <button className="sort-head" key={key} onClick={() => matchupTable.toggle(key)}>{label}<i>{matchupTable.indicator(key)}</i></button>)}</div>{matchupTable.rows.length ? matchupTable.rows.slice(0, 8).map((row) => <div key={`${row.mine}-${row.theirs}`}><strong>{row.mine}</strong><strong>{row.theirs}</strong><span>{row.p}</span><span>{row.w}승 {row.d}무 {row.l}패</span><b>{row.gf}:{row.ga}</b></div>) : <p>두 유저의 리그 맞대결이 아직 없습니다.</p>}</div>
        <p className="class-disclaimer">※ 실제 활성 팀컬러는 API에서 제공하지 않아, 각 경기 선발 카드에서 가장 많이 사용된 시즌 클래스를 스쿼드 성향으로 비교합니다.</p>
      </article>
      <article className="clean-card span-three"><div className="card-heading"><div><p>LEAGUE COMPARISON</p><h2>유저별 공격 효율</h2></div><span>헤더를 눌러 정렬</span></div><div className="comparison-table"><div className="comparison-head"><span>#</span>{([['name','유저'],['matches','경기'],['goalsPerGame','경기당 득점'],['shotAccuracy','유효 슈팅률'],['passAccuracy','패스 성공률']] as Array<[keyof (typeof ranking)[number],string]>).map(([key,label]) => <button className="sort-head" key={key} onClick={() => efficiencyTable.toggle(key)}>{label}<i>{efficiencyTable.indicator(key)}</i></button>)}</div>{efficiencyTable.rows.map((row, i) => <div key={row.name}><b>{i + 1}</b><strong>{row.name}</strong><span>{row.matches}</span><span>{row.goalsPerGame.toFixed(2)}</span><span>{Math.round(row.shotAccuracy)}%</span><span>{Math.round(row.passAccuracy)}%</span></div>)}</div></article>
    </section>
  </main>;
}
