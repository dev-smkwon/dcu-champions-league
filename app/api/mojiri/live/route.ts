...
export async function GET() {
  ...
  try {
    ...
    const lists = await Promise.all(identities.map(({ ouid }) => nexon<string[]>(`/user/match?ouid=${ouid}&matchtype=40&offset=0&limit=50`, key)));
    ...
    const matches: LiveGame[] = details.filter((match) => match.matchDate >= START_AT && match.matchInfo.length === 2 && match.matchInfo.every((info) => memberSet.has(info.nickname))).map((match) => ({
