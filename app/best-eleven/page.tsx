...
export default function BestElevenPage() {
  ...
  const rosterTable = useTriSort<Pick & { gaPerGame: number }>((picks || []).map((pick) => ({ ...pick, gaPerGame: pick.goalsPerGame + pick.assistsPerGame })));
  ...
  return <main className="subpage"><FloatingNav />
    ...
    <section className="page-shell">{!picks ? <LoadingState /> : picks.length === 0 ? <div className="empty-state">포지션별 5경기 이상 출전한 후보가 아직 없습니다.</div> : <>
      ...
      <aside className="best-roster"><div className="best-roster-head"><div><p>SELECTED XI</p><h2>선수별 기록</h2></div><span>카드를 눌러 상세 확인</span></div>
        ...
        <div className="best-table-head">{([['name','선수'],['appearances','출장'],['rating','평점'],['gaPerGame','경기당 G+A'],['goalConversion','결정력']] as Array<[keyof (Pick & {gaPerGame:number}),string]>).map(([key,label]) => <button className="sort-head" key={key} onClick={() => rosterTable.toggle(key)}>{label}<i>{rosterTable.indicator(key)}</i></button>)}</div>{rosterTable.rows.map((pick) => <button type="button" onClick={() => setSelected(pickKey(pick))} className={selected === pickKey(pick) ? "selected" : ""} key={pickKey(pick)}><i>{positionLabels[pick.position]}</i><strong>{pick.name}<small>{pick.owner} · +{pick.grade}</small></strong><span>{pick.appearances}</span><b>{pick.rating.toFixed(1)}</b><span>{pick.gaPerGame.toFixed(2)}</span><span>{pct(pick.goalConversion)}</span></button>)}</aside></div>
