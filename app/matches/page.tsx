...
export default function MatchesPage() {
  ...
  return <main className="subpage"><FloatingNav />
    ...
    <section className="page-shell">
      ...
      {!matches ? <LoadingState /> : <div className="match-list">{visible.map((match) => { const result = match.homeGoals > match.awayGoals ? "win" : match.homeGoals < match.awayGoals ? "loss" : "draw"; return <Link href={`/matches/${match.id}`} className={`match-row result-${result}`} key={match.id}>
        <time>{new Date(`${match.date}+09:00`).toLocaleDateString("ko-KR", { month: "short", day: "numeric", weekday: "short" })}</time>
        <div className="match-side home"><b>{match.home}</b><small>점유율 {match.homePossession}% · 슈팅 {match.homeShots}</small><em className="result-badge">{result === "win" ? "승" : result === "loss" ? "패" : "무"}</em></div>
