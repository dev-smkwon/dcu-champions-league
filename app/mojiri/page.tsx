...
import recordsData from "../../data/mojiri-records.json";
import { MojiriJinxes } from "./jinxes";
import { MojiriMonthSelect } from "./month-select";

type Game = { matchId: string; startedAt: string; home: string; away: string; homeGoals: number; awayGoals: number; note?: string; winner?: string | null; shootout?: { home: number; away: number } };
...
type ElevenPlayer = { owner: string; spId: number; name: string; position: number; grade: number; appearances: number; goals: number; assists: number; rating: number; adjustedRating?: number; groupMean?: number; advancingEligible?: boolean; historicalSelection?: boolean };
...
function buildMojiriStats(tournaments: Tournament[]) {
  ...
  tournaments.forEach((item) => {
    ...
    item.rounds.flatMap((round) => round.series).flatMap((series) => series.games).forEach((game) => {
      ...
      const homeLost = game.winner ? game.winner === game.away : game.homeGoals < game.awayGoals;
      const awayLost = game.winner ? game.winner === game.home : game.awayGoals < game.homeGoals;
...
function SeriesCard({ series, final = false }: { series: Series; final?: boolean }) {
  return <article className={`mojiri-series ${final ? "is-final" : ""}`}>
    ...
    {!!series.games.length && <details className="mojiri-game-fold"><summary>경기 기록 {series.games.length}개 보기</summary><div className="mojiri-games">{series.games.map((game, index) => <Link href={`/matches/${game.matchId}`} className={game.note ? "is-forfeit" : ""} key={game.matchId}><time>{timeLabel(game.startedAt)}</time><span>G{index + 1}</span><strong>{game.home} <b>{game.homeGoals} : {game.awayGoals}</b> {game.away}{game.shootout && <small>승부차기 {game.shootout.home} : {game.shootout.away} · {game.winner} 승</small>}{game.note && <small>{game.note}</small>}</strong><i>›</i></Link>)}</div></details>}
  </article>;
}

export default async function MojiriPage({ searchParams }: { searchParams: Promise<{ month?: string; scope?: string }> }) {
  const params = await searchParams;
  const available = tournaments.filter(t => t.status === "completed").sort((a,b) => b.id.localeCompare(a.id));
  if (!available.length) return <main className="subpage"><FloatingNav/><h1>아직 공개된 모지리 대회가 없습니다.</h1></main>;
  const tournament = available.find(t => t.id === params.month) || available[0];
  const selectedMonth = tournament.id;
  const total = params.scope === "total";
  const scopedTournaments = total ? available : [tournament];
  const scopeLabel = total ? `전체 ${available.length}개 대회 누적` : `${selectedMonth} 대회`;
  const records = total ? recordsData.total : recordsData.months[selectedMonth as keyof typeof recordsData.months];
  const recordTournament = records || tournament;
  const mojiriStats = buildMojiriStats(scopedTournaments);
const leaders = (value: (stat: MojiriStat) => number, direction: "max" | "min" = "max") => {
  ...
  const tournamentGames = scopedTournaments.flatMap(t => t.rounds.flatMap((round) => round.series.flatMap((series) => series.games.map((game, index) => ({ ...game, round: round.name, series: series.label, gameNumber: index + 1 }))))).sort((a, b) => a.startedAt.localeCompare(b.startedAt));
  return <main className="subpage mojiri-page"><FloatingNav />
    <header className="subhero mojiri-hero"><div><p>REVERSE TOURNAMENT · {tournament.id}</p><h1>모지리 토너먼트</h1><span>끝까지 살아남은 패배자, 단 한 명.</span></div><div><Link className="mojiri-draw-link" href="/mojiri/draw">조 추첨식 열기 →</Link><MojiriMonthSelect value={selectedMonth} months={available.map(t => t.id)} scope={total ? "total" : "month"}/><div className="mojiri-crown"><i>🤡</i><span>{Number(tournament.id.slice(5))}월의 모지리</span><strong>{tournament.mojiri}</strong></div></div></header>
    <section className="page-shell mojiri-shell">
      ...
      <section className="mojiri-records" id="records"><header><div><span>MOJIRI ARCHIVE</span><h2>모지리 기록실</h2></div><div><nav className="mojiri-scope" aria-label="기록 집계 범위"><Link aria-current={!total ? "page" : undefined} href={`/mojiri?month=${selectedMonth}&scope=month#records`}>월별</Link><Link aria-current={total ? "page" : undefined} href={`/mojiri?month=${selectedMonth}&scope=total#records`}>토탈</Link></nav><p>{scopeLabel} · 공동 기록은 함께 수상</p></div></header>
        <section className="mojiri-eleven"><header><div><span>REVERSE BEST ELEVEN · 4-3-3</span><h2>모지리 베스트 & 워스트 11</h2></div><p>BEST 11: 패자 진출 경험 필수 · 선수 최소 2경기</p></header><div><MojiriLineup title="모지리 BEST 11" subtitle="패배하며 살아남은 스쿼드만 후보" players={recordTournament.mojiriEleven.best} best/><MojiriLineup title="모지리 WORST 11" subtitle="너무 잘해서 탈락 · 전체 참가자 대상" players={recordTournament.mojiriEleven.worst} best={false}/></div><aside><b>BEST 11 후보 조건</b><span>소속 유저가 최소 한 번 패자로 다음 라운드에 진출해야 하며, 선수는 2경기 이상 출전해야 합니다. 이후 포지션 평균 3경기분을 섞어 표본을 보정합니다. WORST 11은 전체 참가자를 대상으로 기존 기준을 유지합니다.</span></aside></section>
        {total && <MojiriJinxes tournaments={available}/>}
        <h3 className="football-title">기본 축구 기록 <small>{scopeLabel} · 보관된 API 기록</small></h3><div className="mojiri-football-grid">{recordTournament.footballRecords.map((record) => <FootballRecordCard record={record} key={record.id} />)}</div>
        ...
        <details className="mojiri-extra-records"><summary><span>선수·경기 세부 흑역사</span><b>{recordTournament.negativeFootballRecords.length}개 기록 펼쳐보기 ↓</b></summary><div className="mojiri-football-grid negative-football-grid">{recordTournament.negativeFootballRecords.map((record) => <FootballRecordCard record={record} negative key={record.id} />)}</div><p>선수 비율 기록은 최소 2경기입니다. 슛 10회, 패스 20회, 드리블 10회, 태클·공중볼 5회 이상을 대상으로 합니다. 골키퍼 실점은 해당 선수 출전 경기의 팀 실점으로, 개인 책임 실점은 아닙니다. `추정` 표시는 API가 사건 간 직접 연결을 제공하지 않아 근사값으로 계산한 기록입니다.</p></details>
        <section className="mojiri-match-archive"><header><div><span>FULL MATCH TIMELINE</span><h3>모든 경기</h3></div><p>전체 {tournamentGames.length}경기 · 한국시간 · 오래된 경기순</p></header><div>{tournamentGames.map((game) => <Link href={`/matches/${game.matchId}`} className={game.note ? "is-forfeit" : ""} key={game.matchId}><time>{timeLabel(game.startedAt)}</time><span>{game.round}<small>{game.series} · G{game.gameNumber}</small></span><strong>{game.home}</strong><b>{game.homeGoals}<i>:</i>{game.awayGoals}</b><strong>{game.away}</strong><em>{game.shootout ? `승부차기 ${game.shootout.home}:${game.shootout.away} · ${game.winner} 승` : game.note || "상세 보기 ›"}</em></Link>)}</div></section>
      </section>
    </section>
  </main>;
}
