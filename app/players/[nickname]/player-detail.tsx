...
export function PlayerDetail({ nickname }: { nickname: string }) {
  ...
  return <main className="subpage"><FloatingNav />
    ...
    <section className="page-shell detail-grid">
      ...
      <article className="clean-card profile-activity"><div className="card-heading"><div><p>PROFILE</p><h2>활동 요약</h2></div></div><div><span>리그 경기</span><b>{stats.matches}</b><small>7월 1일 이후</small></div><div><span>사용 선수 카드</span><b>{stats.squad.length}</b><small>1경기 이상 출전</small></div><div><span>총 득점</span><b>{stats.goals}</b><small>리그 내부 경기</small></div><div><span>평균 점유율</span><b>{Math.round(stats.possession / Math.max(1, stats.matches))}%</b><small>플레이 성향 참고</small></div></article>
