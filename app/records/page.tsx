...
export default function RecordsPage() {
  ...
  return <main className="subpage records-page"><FloatingNav />
    ...
    <section className="page-shell records-shell">
      ...
      <div className="shot-award-grid">{records.shotAwards.map((award) => <button type="button" onClick={() => setSelectedAward(award.id)} className={`shot-award ${activeAward.id === award.id ? "selected" : ""}`} key={award.title}><i>{award.emoji}</i><div><p>{award.title}</p><h3>{award.user?.name || "아직 없음"}</h3><span>유저 · {award.user?.goals || 0}골 / {award.user?.attempts || 0}회 시도</span></div><div className="award-player"><small>PLAYER</small><b>{award.player?.name || "기록 없음"}</b><span>{award.player?.owner || "-"} · {award.player?.goals || 0}골</span></div></button>)}</div>
      <article className="shot-ranking"><header><div><p>{activeAward.emoji} SHOT TYPE RANKING</p><h2>{activeAward.title} 선수 랭킹</h2></div><span>득점 우선 · 동률 시 시도 횟수</span></header><div className="shot-ranking-head"><span>#</span><span>선수</span><span>유저</span><span>득점</span><span>시도</span><span>성공률</span></div>{activeAward.rankings.length ? activeAward.rankings.slice(0, 10).map((row, index) => <div className="shot-ranking-row" key={`${activeAward.id}-${row.owner}-${row.spId}`}><b>{index + 1}</b><strong>{row.name}</strong><span>{row.owner}</span><em>{row.goals}</em><span>{row.attempts}</span><span>{Math.round(row.conversion * 100)}%</span></div>) : <p className="no-record">해당 유형의 슈팅 기록이 아직 없습니다.</p>}</article>
