...
export default function RecordsPage() {
  ...
  return <main className="subpage records-page"><FloatingNav />
    ...
    <section className="page-shell records-shell">
      ...
      <div className="records-intro playmaking-title"><div><p>PASS TO GOAL LAB</p><h2>한 번에 수비를 벗기는 법</h2></div><span>도움·득점 좌표 기반 추정 · 미득점 기회 창출은 제외</span></div>
      <div className="playmaking-groups">{(records.playmakingGroups || []).map((group) => <section className={`playmaking-group-list tone-${group.tone}`} key={group.id}><div className="playmaking-group-heading"><div><p>{group.eyebrow}</p><h3>{group.title}</h3></div><span>{group.description}</span></div><div className="record-board-grid">{group.records.map((record) => <article className={`record-board playmaking-board board-${group.tone}`} key={record.id}><header><i>{record.emoji}</i><div><h3>{record.title}</h3><p>{record.description}</p></div><small className="board-mode-badge">추정</small></header><div>{record.rankings.length ? record.rankings.slice(0, 10).map((row, index) => <div className="record-row" key={`${record.id}-${row.name}-${row.meta}-${index}`}><em>{index + 1}</em><strong>{row.name}<small>{row.meta}</small></strong><b>{row.value.toLocaleString("ko-KR", { maximumFractionDigits: 2 })}<small>{record.unit}</small></b></div>) : <p className="no-record">조건을 충족한 기록이 아직 없습니다.</p>}</div></article>)}</div></section>)}</div>
